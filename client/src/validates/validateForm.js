...
///[-a-zA-Z0-9@:%._~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_.~#?&//=]*)?/gi
  if(!/^(ftp|http|https):\/\/[^ "]+$/.test(stateValidate.image) /*|| !/(\.jpe?g|.png|.svg)$/i.test(stateValidate.image)*/){
    errors.image = "Invalid url"
  }

  if (!stateValidate.name) {
    errors.name = 'Name is required';
} else if(stateValidate.name.length > 50){
  errors.name = 'Name to long';
}else if (stateValidate.name[0]===" "){
  errors.name = 'Blank spaces are not allowed at the beginning';
} else if (!/^[A-ZÑ\s]+$/i.test(stateValidate.name) || stateValidate.name.endsWith(" ")) {
  ...
  } else if(parseInt(stateValidate.height_min[0]) === 0){
    errors.height_min = '0 is not allowed at the beginning';
...
} else if(parseInt(stateValidate.height_max[0]) === 0){
  errors.height_max = '0 is not allowed at the beginning';
  ...
  } else if(parseInt(stateValidate.weight_min[0]) === 0){
    errors.weight_min = '0 is not allowed at the beginning';
  ...
  } else if(parseInt(stateValidate.weight_max[0]) === 0){
    errors.weight_max = '0 is not allowed at the beginning';
  ...
  } else if(parseInt(stateValidate.life_span_min[0]) === 0){
    errors.life_span_min = '0 is not allowed at the beginning';
  ...
  } else if(parseInt(stateValidate.life_span_max[0]) === 0){
    errors.life_span_max = '0 is not allowed at the beginning';
